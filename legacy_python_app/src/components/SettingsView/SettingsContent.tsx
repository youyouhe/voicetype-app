...
export const ASRSettings: React.FC = () => {
  const [selectedService, setSelectedService] = useState<ServiceProvider>(ServiceProvider.Groq);
    ...
    <div className="max-w-3xl animate-in fade-in duration-500">
      ...
      <section className="bg-white rounded-xl border border-gray-200 p-6 mb-6 shadow-sm">
        ...
        <div className="grid gap-4">
          ...
          />
          <ServiceOption
            id={ServiceProvider.Groq}
            title="Groq Cloud"
            description="Ultra-fast cloud inference. Requires API Key."
            icon={<Cloud className="w-6 h-6" />}
            selected={selectedService === ServiceProvider.Groq}
            onSelect={() => setSelectedService(ServiceProvider.Groq)}
          />
          <ServiceOption
            id={ServiceProvider.SiliconFlow}
            title="SiliconFlow"
            description="Optimized for Chinese. Auto-punctuation support."
            icon={<Globe className="w-6 h-6" />}
            selected={selectedService === ServiceProvider.SiliconFlow}
            onSelect={() => setSelectedService(ServiceProvider.SiliconFlow)}
          />
        </div>
      </section>
