...
impl AudioRecorder {
    ...
    pub fn stop_recording(&mut self) -> Result<Cursor<Vec<u8>>, VoiceError> {
        if !self.recording {
        ...
        let wav_bytes = self.audio_to_wav(&self.audio_data)?;
        self.audio_data.clear();

        println!("WAV conversion complete, {} bytes", wav_bytes.len());
